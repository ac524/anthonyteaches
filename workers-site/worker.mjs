...
<div class="${"window svelte-1sly5wv"}">${validate_component(Game, "RPSGame").$$render($$result, {}, {}, {})}
</div>`;
    });
  }
});
...
var init_tic_tac_toe_svelte = __esm({
  ".svelte-kit/output/server/entries/pages/playground/tic-tac-toe.svelte.js"() {
    ...
    GameTile = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      ].join(" ").trim()}"${add_attribute("title", label, 0)} ${winningTiles && !isWinning ? "disabled" : ""}>${tile.value ? `<span class="${"svelte-4bkcom"}"><span>${escape(tile.value)}</span></span>` : `${``}`}
</button>`;
    });
    Tile = class {
    ...
    Game2 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      return `<div class="${"game-container svelte-1q5bu8f"}"><div class="${"box message svelte-1q5bu8f"}">${winningTiles ? `<span>Reset the game to play again</span>` : `<span>Click the tiles to play the game</span>`}</div>
    <div class="${"row svelte-1q5bu8f"}"><div class="${[
        "col box text-center message row row-ai-center text-wrap-no svelte-1q5bu8f",
        winningTiles ? "is-win" : ""
        ...
        "col box fs-larger row row-ai-center svelte-1q5bu8f",
        (turnFocused === "X" ? "is-x" : "") + " " + (turnFocused === "O" ? "is-o" : "") + " " + (winningTiles ? "is-win" : "")
      ].join(" ").trim()}" style="${"max-width:40px;border-left:2px solid #FFF"}"><span class="${"col text-center svelte-1q5bu8f"}">${escape(turnFocused)}</span></div></div>
    <div class="${"game-box svelte-1q5bu8f"}"><div class="${"game-box--inner svelte-1q5bu8f"}"><div class="${"game-grid svelte-1q5bu8f"}">
                ${each(tiles, (tile) => {
...
<div class="${"window svelte-1f34dlt"}">${validate_component(Game2, "TicTacToeGame").$$render($$result, {}, {}, {})}
</div>`;
    });
  }
});
...
var postId_svelte_exports = {};
__export(postId_svelte_exports, {
  default: () => U5BpostIdu5D,
  load: () => load2
});
async function load2({ params }) {
  return { props: { postId: params.postId } };
}
var U5BpostIdu5D;
